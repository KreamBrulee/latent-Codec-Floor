...
| **Q-12** | Motion energy (mean flow magnitude) is blind to *timing*. The slow-pan proxy reconstruction keeps its mean motion (0.590 vs 0.604 px/frame, "97.7% retained") but its per-pair motion becomes a sawtooth between keyframes: per-pair coefficient of variation 0.016 → 0.240 (found 2026-09-25, `docs/figures/fig2`) | "Motion retained" can report near-total preservation for motion the codec has made jerky; §V and Table III wording overstate what the metric shows | Report per-pair motion dispersion alongside motion energy, and/or rely on FVMD, which uses velocity *and acceleration* (F-2) |
