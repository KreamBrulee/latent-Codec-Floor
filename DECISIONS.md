...
| 2026-09-25 | L-3 | Paper Table II floor column relabelled **FVMD → motion lost (%)** and filled from the LTX pilot (9.5 / 17.5 / 13.8); §III.A ratio 1.8× (CI [0.61, 6.77], includes 1). All numbers added to the paper are yellow-highlighted tracked changes (user request) | Waiting for FVMD integration (F-2): user preferred pilot numbers now | FVMD integrated: restore the FVMD column, keep motion lost as a secondary column. End-to-end must use the same metric (R2), and needs source-conditioned generation (Q-9) |

**LTX pilot result (record `runs/ltx_floor_20260925T181804Z.json`; LTX-Video 0.9.5 VAE @ `e58e28c3`, fp32, untiled, timestep 0, posterior mode; 15 derf clips, 33×768×416; strata by p99, n = 5 each):** Δ motion energy / Δ warping error, mean [95% bootstrap CI]: slow_pan −9.5% [−19.6, −2.9] / +13.5% [−9.2, +38.7]; fast_action −17.5% [−30.4, −7.4] / −10.0% [−27.5, +4.6]; fine_texture −13.8% [−34.5, +2.9] / **−35.4% [−38.9, −32.0]**. Round trip bit-identical on repeat (G-3). Peak VRAM 7.1 GB (VAE), 8.2 GB (RAFT); running both at once overflowed 12 GB and spilled 4.4 GB to system RAM, so the script runs them in separate phases.
